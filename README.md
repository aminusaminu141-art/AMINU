...
* **Student Portal:** View attendance history, finalized academic report cards, and class position rankings.
...
2. Open your database administration tool (such as [phpMyAdmin](http://localhost/phpmyadmin)) and create a new database named `amsuf_db`:
   ...
   ```
3. Locate the `server/.env` file and verify your MySQL credentials. The default configuration is:
   ```env
   PORT=5000
   DB_HOST=localhost
   DB_USER=root
   ...
   ```
4. Navigate to the `server/` directory and run the database setup and seeding script to populate the tables and default accounts:
   ```bash
   cd server
   npm install
   node seed.js
